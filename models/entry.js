...
const updateEntries = async (title) => {
    const {content,email,category} = entry;
    let client,result;
    try{
        client = await pool.connect(); // Espera a abrir conexion
        const data = await client.query(`UPDATE entries(title,content,id_author,category)
                                    VALUES (($1),($2),
                                    (SELECT id_author FROM authors WHERE email=$3),($4))`
                                    ,[title,content,email,category])
        result = data.rowCount
...
// Pruebas
    ...
    // updateEntries("noticia desde Node")
    // .then(data=>console.log(data))
/*
getAllEntries()
.then(data=>console.log(data))
...
// let newEntry = {title:"noticia desde Node",content:"va a triunfar esto",
// email:"alejandru@thebridgeschool.es",category:"sucesos"}
