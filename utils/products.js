...
const getProductById = async (id) => {
    try{
        ...
        return [products];//Pinta datos en el pug. Aquí hemos metido data en un objeto para  que con la plantilla del pug lo coja.
      }catch(error){
...
const getAllProducts = async () => {
    try{
        ...
        let products = await response.json();//{}
        return products;//Pinta datos en el pug
      }catch(error){
        console.log(`ERROR: ${error.stack}`);
        return [];
      }
